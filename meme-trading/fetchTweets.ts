...
export const fetchTweets = async (username: string): Promise<FetchTweetsResult> => {
  try {
    ...
    const timelineResponse = data?.data?.user_result?.result?.timeline_response?.timeline?.instructions?.find(
    ...
    const tweets: Tweet[] = timelineResponse.entries
      .map((entry: any) => {
        ...
        if (tweetResult) {
          return {
            contents: tweetResult.legacy?.full_text ?? tweetResult.core?.user_result?.result?.legacy?.description ?? "",
      ...
      .filter((tweet: Tweet): tweet is Tweet => Boolean(tweet && tweet.contents && tweet.id && tweet.createdAt))
